...
function delete_old_entires(){
  global $wpdb;
  $ids = $wpdb->get_col( "SELECT ID FROM {$wpdb->prefix}mvi_webinar_registration" );
  if ( !empty($ids) ) {
  	...
  	if ( $query->have_posts() ) {
  		while ( $query->have_posts() ) {
  			$query->the_post();
        $post_id = get_the_ID();
        $export_count = rwmb_meta( PLUGIN_PREFIX . 'export_count', ['storage_type' => 'custom_table', 'table' => WPDB_PREFIX . 'mvi_webinar_registration'], $post_id);
        $date_time = rwmb_meta( PLUGIN_PREFIX . 'date_time', ['storage_type' => 'custom_table', 'table' => WPDB_PREFIX . 'mvi_webinar_registration'], $post_id);
