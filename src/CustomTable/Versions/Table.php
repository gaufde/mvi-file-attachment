...
abstract class Table implements TableVersionInterface
{
    ...
    {
        global $wpdb;
        $custom_table_id = $wpdb->prefix . \MVIWebinarRegistrationBase::PLUGIN_PREFIX . "submissions";
    ...
    {
        return array(\MVIWebinarRegistrationBase::PLUGIN_PREFIX . 'email');
    }
     ...
     * @return bool
     */
    public static function set_db_ver($new_db_version)
    {
        $columns = static::define_table_columns();
        $table_name = static::get_id();
        $old_db_version = \MVIWebinarRegistration\CustomTable::get_db_ver();
        $result = false;
        try {
            ...
            if ($columns_match) {
                ...
                if ($old_db_version == $new_db_version) {
                    ...
                    update_option(\MVIWebinarRegistrationBase::PLUGIN_PREFIX . "db_version", $new_db_version);
                    $result = true;
                    throw new \Exception("The table db version number for {$table_name} was updated from ({$old_db_version}) to ({$new_db_version}).");
                } else {
                    add_option(\MVIWebinarRegistrationBase::PLUGIN_PREFIX . "db_version", $new_db_version);
                    $result = true;
